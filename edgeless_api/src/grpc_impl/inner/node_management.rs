...
impl NodeManagementClient {
    ...
    async fn try_connect(&mut self) -> anyhow::Result<()> {
        if self.client.is_none() {
            ...
            self.client =
                Some(crate::grpc_impl::api::node_management_client::NodeManagementClient::new(channel).max_decoding_message_size(usize::MAX).max_encoding_message_size(usize::MAX));
        }
        Ok(())
    }
